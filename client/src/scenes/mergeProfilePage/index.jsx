...
const MergeProfilePage = () => {
    ...
    return (
        <Box>
            <Navbar/>
            <Box display="flex" justifyContent="space-between" marginTop="2rem" gap="2rem">
                <Box flexBasis={isNonMobileScreens ? "26%" : undefined} paddingLeft="2rem" paddingRight="2rem">
                    <LinksWidget/>
                    <Box m="2rem 0"/>
                    {myProfile && <PersonalNavigator onMyDraftsClick={handleMyDraftsClick}/>}
                </Box>
                <Box flexBasis={isNonMobileScreens ? "42%" : undefined}>
                    {showMyDrafts ? <MyDraftsWidget/> : <SummaryWidget/>}
                </Box>
                <Box flexBasis={isNonMobileScreens ? "26%" : undefined} paddingRight="2rem" mb={5}>
