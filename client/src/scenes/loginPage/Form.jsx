...
const Form = () => {
  ...
  const register = async (values, onSubmitProps) => { 
    ...
    const savedUserResponse = await fetch( // we will send form data to backend
      "http://localhost:3001/auth/register", // this is the register url of backend
      {
        method: "POST",
        body: formData,
      }
  ...
  const login = async (values, onSubmitProps) => {
    setFormError(null); // we will set formError to null before submitting the form
    const loggedInResponse = await fetch("http://localhost:3001/auth/login", { // we will send form data to backend
      method: "POST",
